  ...
  module Helpers
    ...
    module ViewHelpers
      ...
      def skip_action_link(link, record = nil)
        (link.security_method_set? or controller.respond_to? link.security_method) and !(controller.method(link.security_method).arity == 1 ? controller.send(link.security_method, record) : controller.send(link.security_method))
      end
